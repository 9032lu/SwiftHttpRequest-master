...
extension ViewController {
    ...
    func loadDataByMoay() {
        ...
        HttpRequest.loadData(API: DMAPI.self, target: .other1(param: "asd"), cache: true, success: { (json) in
            ...
            TAlert.show(type: TAlert.AlertType.warning, text: model?.data.msg ?? "")
...
//        HttpRequest.loadData(API: DMAPI.self, target: .rankList, cache: true, success: { (json) in
//            let decoder = JSONDecoder()
//            let model = try? decoder.decode(DMModel.self, from: json)
//            self.model = model
