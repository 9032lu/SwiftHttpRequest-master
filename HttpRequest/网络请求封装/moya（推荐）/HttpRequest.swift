        ...
        provider.request(target) { result in
            ...
            case let .success(response):
                ...
                case NET_STATE_CODE_LOGIN:
                    //请重新登录
                    if let failureBlack = failure {
                        failureBlack(model.data.tokenStatus ,model.data.msg)
                    }
                    alertLogin(model.data.msg)
                    break
                default:
                    //其他错误
                    failureHandle(failure: failure, stateCode: model.data.tokenStatus, message: model.data.msg)
                    break
                }
            // ********************
...
extension URLRequest {
    ...
    private var commonParams: [String: Any] {
        //所有接口的公共参数添加在这里
        return ["libraryId":"2","userId":"2356","token":"239e4d7ba1bfe07ae2364fd834058cdd","page":"1","cardNumber":"wangfeng","keyword":"傅玉璋","batchId":"2"]
    }
